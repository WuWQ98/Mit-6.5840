	...
	for idx := range rf.peers {
		...
		go func(idx int) {
			...
			if reply.Term > rf.currentTerm {
				...
				if reply.VoteGranted && rf.loadRole() == CANDIDATE && args.Term == rf.currentTerm { // args.Term == rf.currentTerm 用来判断rpc请求返回是否跨任期，即是否过期
					voteCnt++
	...
	if args.Term >= rf.currentTerm {
		if rf.votedFor.Term == args.Term && rf.votedFor.CandidateId != args.CandidateId { // 相同任期内已经投票给其他candidate
			...
			if rf.checkCandidateLog(args.LastLogIndex, args.LastLogTerm) { // candidate的log未过时
				...
				rf.votedFor = VoteForInfo{args.Term, args.CandidateId}
			} else { // candidate的log过时
				// DPrintf("%v ---- 节点：%d 收到 %d 过时log的投票请求，拒绝投票\n", time.Now(), rf.me, args.CandidateId)
				reply.VoteGranted = false
			}
		}
		if rf.loadRole() != CANDIDATE { //避免以下情况：网络故障的旧leader节点恢复，当前集群中因为无leader节点而正在选举，rv rpc的任期同步功能直接将故障恢复旧leader不经过选举变为新leader
